...
export async function loadSkillsConfig(
  ...
  candidatePaths.push(
    path.resolve(getModuleDir(import.meta.url), "../skills.config.json"),
  );
  ...
  for (const configFilePath of candidatePaths) {
    try {
      ...
      if (!parsed || !Array.isArray(parsed.skills)) {
        throw new Error(
          `Invalid configuration format in '${configFilePath}': 'skills' must be an array.`,
        );
      }

      // Validate individual skill entries and bundles with backward compatibility
      const validatedSkills: SkillConfig[] = parsed.skills.map(
        (item: any, idx: number) => {
          if (!item.name || typeof item.name !== "string") {
            throw new Error(
              `Skill entry at index ${idx} missing 'name' string property.`,
            );
          }
          if (!item.repository || typeof item.repository !== "string") {
            throw new Error(
              `Skill entry '${item.name}' missing 'repository' string property.`,
            );
          }
      ...
      if (err instanceof SyntaxError) {
        throw new Error(
          `JSON syntax error in '${configFilePath}': ${err.message}`,
        );
      }
      lastError = err;
    }
...
export async function removeSkillsFromConfig(
  ...
  try {
    ...
    const namesToRemove = new Set(
      skillNames.map((n) => n.trim().toLowerCase()),
    );
    const initialCount = parsed.skills.length;

    parsed.skills = parsed.skills.filter(
      (item) =>
        item && item.name && !namesToRemove.has(item.name.trim().toLowerCase()),
    );

 ...
 *
 * Cursor IDE stores MCP server definitions in the user profile `mcp.json`:
 *   - Windows: %APPDATA%\Cursor\User\mcp.json
 *   - macOS:   ~/Library/Application Support/Cursor/User/mcp.json
 *   - Linux:   ~/.config/Cursor/User/mcp.json
 *
 * Cursor uses the same configuration structure as VS Code.
 */
export function getCursorMcpConfigPath(): string {
  ...
  return path.join(configHome, "Cursor", "User", "mcp.json");
}
 ...
 * Cursor uses the `servers` top-level key (same as VS Code).
 *
